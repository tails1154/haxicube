...
CC_API void HacksComp_Init(struct HacksComp* hacks) {
...
CC_API void HacksComp_Update(struct HacksComp* hacks) {
...
CC_API void HacksComp_SetFlying(struct HacksComp* hacks, cc_bool flying) {
	if (hacks->Flying == flying) return;
	hacks->Flying = flying;
	Event_RaiseVoid(&UserEvents.HacksStateChanged);
}

CC_API void HacksComp_SetNoclip(struct HacksComp* hacks, cc_bool noclip) {
